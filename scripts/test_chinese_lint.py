...
with tempfile.TemporaryDirectory() as base:
    ...
    check("professional source", not LINT.lint_file(good, "source", "general", None, 280))
    check("Chinese comment", any("comments" in item[1]
                                 for item in LINT.lint_file(
                                     bad_comment, "source", "general", None, 280)))
    prose_findings = LINT.lint_file(bad_prose, "prose", "general", None, 280)
    ...
    check("overlay body", not LINT.lint_file(
    ...
    check("overlay commit message", not LINT.lint_file(
    ...
    check("data file Chinese length", any(
        "paragraph" in item[1] for item in LINT.lint_file(data, "prose", "general",
                                                          None, 280)))
    ...
    wrong_findings = [item for item in LINT.lint_file(
    ...
    pr_inventory.write_text(
        ...
        "- 補充測試案例。\n\n"
        "目前所有功能均已完成。\n")
    inventory_findings = LINT.lint_file(
    ...
    check("required PR template excluded", not LINT.lint_file(
    ...
    mt_findings = LINT.lint_file(machine_translation, "prose", "general", None, 280)
    ...
    check("all mode checks Pandoc fenced comments", any(
        "comments" in item[1]
        for item in LINT.lint_file(combined, "all", "general", None, 280)))
    table = root / "table.md"
    table.write_text("| 欄位 | 說明 |\n|---|---|\n| value | " + "字" * 400 + " |\n")
    check("Markdown table cell length", any(
        "paragraph" in item[1]
        for item in LINT.lint_file(table, "all", "general", None, 280)))
    ...
    check("measured improvement", not any(
        ...
        for item in LINT.lint_file(measured, "prose", "general", None, 280)))
    ...
    check("adjacent ASCII punctuation", len([
        item for item in LINT.lint_file(punctuation, "prose", "general", None, 280)
    ...
    regional_findings = LINT.lint_file(regional, "prose", "general", None, 280,
                                       "zh-TW", True)
    ...
    check("regional check is opt-in", not any(
        "regional vocabulary" in item[1] for item in LINT.lint_file(
